...
data <- map(2:6,                                                                 # For each year in it's own sheet
    ~{readxl::read_excel("./Data/EU fish/FDI-catches-by-country.xlsx", sheet = .x) %>% # Import data
    filter(`sub-region` %in% c("27.1.A", "27.1.B", "27.2.A", "27.2.B"))}) %>%    # Limit to regions of interest
