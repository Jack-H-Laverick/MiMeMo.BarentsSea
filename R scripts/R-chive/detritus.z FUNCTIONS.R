...
get_bio   <- function(file)                    { 
  print(str_glue("getting Detrital Nitrogen from {file}"))
  nc_raw <- nc_open(file)                                                    # Open up a netcdf file to see it's raw contents (var names)
  nc_DET <- ncvar_get(nc_raw, "DET", start3D, count3D)                       # Extract a matrix for the variable
  nc_close(nc_raw)                                                           # You must close an open netcdf file when finished to avoid data loss
  output %>%                                                                 # Grab the tidied dataframe of lat-longs
    mutate(S_Detritus = as.numeric(stratify(nc_DET, Shallow_mark, sw)),           # Collapse shallow DIN into 2D and convert to long format
           M_Detritus = as.numeric(stratify(nc_DET, Middle_mark, mw)),
           D_Detritus = as.numeric(stratify(nc_DET, Deep_mark, dw)))      # Collapse, reshape and append deepwater data
}    # Pull detritus from P files 

summarise_months <- function(month, get, ) {
    ...
    Monthly <- month %>%                                                     # Take the year
      ...
      summarise(S_Detritus = mean(S_Detritus, na.rm = TRUE),  
                M_Detritus = mean(M_Detritus, na.rm = TRUE),      
                D_Detritus = mean(D_Detritus, na.rm = TRUE))  %>%                    # Calculate monthly average at each pixel
      ungroup()
